...
/* ── Constants ───────────────────────────────────────────────── */
const THUMB = { SCALE: 0.22, W: 1280, H: 900 } as const
...
function SiteCard({ site, index, onRequestDelete }: { site: Site; index: number; onRequestDelete: (id: string, name: string) => void }) {
  ...
  return (
    <div ref={ref} onMouseEnter={onEnter} onMouseLeave={onLeave}
      ...
      {/* Thumbnail */}
      {site.html ? (
        <div style={{ height: '160px', borderBottom: '1px solid rgba(255,255,255,0.06)', borderRadius: '18px 18px 0 0', overflow: 'hidden', position: 'relative', backgroundColor: '#000' }}>
          <div style={{ width: `${THUMB.W * THUMB.SCALE}px`, height: `${THUMB.H * THUMB.SCALE}px` }}>
            <iframe
              srcDoc={site.html}
              style={{ width: `${THUMB.W}px`, height: `${THUMB.H}px`, border: 'none', transform: `scale(${THUMB.SCALE})`, transformOrigin: 'top left', pointerEvents: 'none' }}
              title={`${site.name} preview`}
              sandbox="allow-same-origin"
            />
          </div>
          <div style={{ position: 'absolute', inset: 0, transition: 'background 300ms', background: hovered ? `linear-gradient(to bottom, transparent 30%, ${site.color}22 100%)` : 'linear-gradient(to bottom, transparent 50%, rgba(17,17,17,0.95) 100%)' }} />
        </div>
      ) : (
        <div style={{ height: '160px', background: `${site.color}0d`, borderBottom: '1px solid rgba(255,255,255,0.06)', borderRadius: '18px 18px 0 0', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '0.375rem' }}>
          ...
          <div ref={menuRef} style={{ position: 'relative' }}>
            <button onClick={() => setMenuOpen(prev => !prev)}
              style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.3)', cursor: 'pointer', fontSize: '1.1rem', padding: '0 0.25rem', lineHeight: 1 }}>
              ···
            </button>
            {menuOpen && (
